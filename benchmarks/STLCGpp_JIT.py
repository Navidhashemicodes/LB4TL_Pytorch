...
# Track total times
def robustness(x):
    global total_formula_time, total_robustness_time

    x = torch.flip(x, (0,)).unsqueeze(0)

    formula1, _ = eventually_goal1_then_eventually_goal2(x, x.shape[1]-1)
    formula2, _ = always_safe(x)
...
for i in range(0,20):
    ...
    sample_trajectory = torch.randn(T + 1, 3)

    # Wrap robustness in a dummy nn.Module so it can be traced

    robustness_module = RobustnessModule()

    # Trace the computation graph
    traced = torch.jit.trace(robustness_module, sample_trajectory)
    ...
    for i in tqdm(range(Epochs)):
        ...
        objective_value = traced(trajectory)
...
import numpy as np
savemat("Times_stlcgpp.mat", {"Times": np.array(Times, dtype=np.float64)})